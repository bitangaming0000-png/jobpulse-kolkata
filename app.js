...
(function initHeader(){
  ...
  function tick(){
    const now = new Date();
    dt && (dt.textContent = now.toLocaleString());
  }
...
async function getPosts(){
  // 1) Try fresh local cache
  const local = loadCache();
  if (local) return local;

  // 2) Try serverless function
  try{
    ...
    // Show soft notice on banner if exists
...
const LOCATIONS = [
  "Kolkata","Howrah","Siliguri","Asansol","Durgapur","Haldia","Kharagpur",
  "Burdwan","Jalpaiguri","Malda","Bankura","Purulia"
];
...
function detectLoc(title, desc){
  ...
  for (const L of LOCATIONS){
    if (new RegExp(`\\b${L}\\b`,"i").test(hay)) return L;
  }
  return "";
}
function enrich(p){
...
function cardHTML(post, fromPage){
  ...
  const badge =
    post.kind === "govt" ? '<span class="badge govt">Govt</span>' :
...
/* Blank-state helper so UI never looks empty */
...
/* Sidebar lists (only used on Home) */
function fillScrollableList(containerId, arr, fromTag){
  const el = document.getElementById(containerId);
  if (!el) return;
  el.innerHTML = `<ul>${
    arr.map(p=>`<li><a href="post.html?id=${encodeURIComponent(p.id)}&from=${fromTag}">${safe(p.title)}</a></li>`).join("")
  }</ul>`;
}
...
/* Breaking banner (top red bar) */
function renderBreakingBanner(){
  const el = document.getElementById("breakingBanner");
  if (!el) return;
  const b = NEWS[0];
  if (b){
    el.innerHTML = `🚨 <a href="post.html?id=${encodeURIComponent(b.id)}&from=news">${safe(b.title)}</a>`;
  } else {
    el.textContent = "No breaking news available right now.";
  }
}
...
function initPost(){
  ...
  // Paint from session cache first (instant)
