// jobful-style multi-endpoint API on Netlify Functions
// Endpoints:
//   /.netlify/functions/jobful                -> { jobs, news, exams }
//   /.netlify/functions/jobful?type=jobs      -> jobs[]
//   /.netlify/functions/jobful?type=news      -> news[]
//   /.netlify/functions/jobful?type=exams     -> exams[]
//   /.netlify/functions/jobful?source=<key>   -> items from one source (see sources.json)
//   Optional: &region=IN-WB to filter by region tag
...
const BUILTIN = {
  remotive: { enabled: true, type: "job",  name: "Remotive",               url: "https://remotive.com/feed",                        region: "GLOBAL" },
  wwr:      { enabled: true, type: "job",  name: "We Work Remotely",       url: "https://weworkremotely.com/remote-jobs.rss",       region: "GLOBAL" },
  highered: { enabled: true, type: "job",  name: "HigherEdJobs",           url: "https://www.higheredjobs.com/rss/",                region: "GLOBAL" },
  ucr:      { enabled: true, type: "news", name: "Undercover Recruiter",   url: "https://theundercoverrecruiter.com/feed",          region: "GLOBAL" },
  acara:    { enabled: true, type: "news", name: "Acara Solutions (Blog)", url: "https://acarasolutions.com/blog/category/job-seeker/feed", region: "GLOBAL" }
};
...
async function buildAll(SOURCES){
  ...
  for (const [key, src] of Object.entries(SOURCES)){
    try{
      const arr = await fetchOne(key, src);
      if (src.type === "job") jobs.push(...arr);
      else news.push(...arr);
      arr.forEach(p=>{
